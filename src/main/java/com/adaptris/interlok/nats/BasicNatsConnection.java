 ...
 * <p>
 * This connection assumes that {@code Nats#connect(String)} is sufficient configuration for connecting to your NATS server (i.e. everything
 * you need is configurable via the URL). Generally put your 'closest' NATS server as the URL, since it will discover other servers after it
 * initially connects and use those as required.
 * </p>
 * <p>
 ...
 * reconnection that happens behind the seens from the NATS core java client. By default outgoing messages will be buffered in memory;
 * pending reconnection and subsequent redelivery.
 * </p>
 *
...
public class BasicNatsConnection extends NatsConnection {
  ...
  /**
   * The URL to the nats server assuming that {@code Nats#connect(String)} is sufficient configuration for connecting to your NATS server.
   *
   * <p>
   * The Java client generally expects URLs of the form {@code nats://hostname:port} but has support for the additional forms (it can be a
